...
def run_training(cfg):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for i, batch in enumerate(dataloader):
            ...
            x = batch['x'].to(device)  # (B, TS, C, H, W)
            y_ch = batch['y_ch'].to(device)  # (B, C, H, W)
            y_sem = batch['y_sem'].to(device)  # (B, TS, C, H, W)

            # https://github.com/mpapadomanolaki/multi-task-L-UNet/blob/main/code/main.py
            F_ch, F_sem1, F_sem2 = net(x)

            F_sum = F_ch + F_sem1
            F_ch2 = F_sem2 - F_sem1
            mychange = torch.nn.ReLU()(y_sem[:, -1] - y_sem[:, 0])

            loss_ch = criterion(torch.sigmoid(F_ch), y_ch)
            loss_sem1 = criterion(torch.sigmoid(F_sem1), y_sem[:, 0])
            loss_sem2 = criterion(torch.sigmoid(F_sem2), y_sem[:, -1])
            loss_sum = criterion(torch.sigmoid(F_sum), y_sem[:, -1])
            loss_ch2 = criterion(torch.sigmoid(F_ch2), mychange)

            # weights for SpaceNet7 0.8, 0.05, 0.05, 0.05, 0.05
            loss = 0.8 * loss_ch + 0.05 * loss_sem1 + 0.05 * loss_sem2 + 0.05 * loss_sum + 0.05 * loss_ch2

            loss.backward()
            optimizer.step()

            loss_set.append(loss.item())
            change_pixels += y_ch.sum().item()
            total_pixels += y_ch.numel()
